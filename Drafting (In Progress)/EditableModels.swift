...
extension Manageable where Self: Menuable {
    ...
    func manageSection(
        ...
        if !isBlocked {
            if canFollow {
                Button(action: {
                    Task { [follow = appActions.follow, weak fetcher] in
                        follow(name)
                ...
                Button(action: {
                    Task { [unfollow = appActions.unFollow, weak fetcher] in
                        unfollow(name)
                        await fetcher?.updateIfNeeded(forceReload: true)
                    }
                }, label: {
            ...
            if isPinned {
                Button(action: {
                    Task { [unpin = appActions.unPin, weak fetcher] in
                        unpin(name)
                        await fetcher?.updateIfNeeded(forceReload: true)
                    }
                }, label: {
                ...
                Button(action: {
                    Task { [pin = appActions.pin, weak fetcher] in
                        pin(name)
                        await fetcher?.updateIfNeeded(forceReload: true)
                    }
                }, label: {
            ...
            Menu {
                Button(role: .destructive, action: {
                    Task { [block = appActions.block, weak fetcher] in
                        block(name)
                        await fetcher?.updateIfNeeded(forceReload: true)
                    }
                }, label: {
            ...
            if book.blocked.contains(name) {
                Button(action: {
                    Task { [unblock = appActions.unBlock, weak fetcher] in
                        unblock(name)
                        await fetcher?.updateIfNeeded(forceReload: true)
                    }
                }, label: {
