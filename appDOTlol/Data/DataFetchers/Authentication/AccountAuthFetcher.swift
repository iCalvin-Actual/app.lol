...
private let logger = Logger(subsystem: "AccountAuthFetcher", category: "auth")

@Observable
final class AccountAuthFetcher: Sendable {
    ...
    let client: AppClient.Identifiers
    let interface: OMGInterface
    
    init(session: WebAuthenticationSession, client: AppClient.Identifiers, interface: OMGInterface, authenticate: @escaping @Sendable (APICredential) -> Void) {
