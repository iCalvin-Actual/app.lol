...
struct ListView<T: Listable>: View {
    ...
    func usingRegular(_ width: CGFloat) -> Bool {
        .usingRegularTabBar(sizeClass: horizontalSize, width: width)
    }
    ...
    var body: some View {
        ...
        .when(T.sortOptions.count > 1 || applicableFilters.count > 1, makeContent: {
            $0.toolbar {
                ToolbarItemGroup {
    ...
    @ViewBuilder
    func detailBody(_ proxy: GeometryProxy) -> some View {
    ...
    private struct ListableView: View {
        @Environment(\.addressBook) var addressBook
        @Environment(\.presentListable) var present
